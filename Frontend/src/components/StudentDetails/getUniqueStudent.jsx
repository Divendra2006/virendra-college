...
function GetUniqueStudent() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      try {
        ...
        setSubmitStatus('student fetched successfully');
      } catch (error) {
        setSubmitStatus('student not found');
        setStudentData([]);
      }
    } else {
  ...
  const handleDeleteClick = async (studentId) => {
    const isConfirmed = window.confirm(`Are you sure you want to delete student ${formData.fullName}?`);
  ...
  return (
    <div className="min-h-screen pt-7 bg-gradient-to-r from-blue-50 to-blue-100 pb-10"> 
      <form onSubmit={handleSubmit} className="max-w-lg mx-auto bg-white shadow-xl p-8 rounded-lg mt-10">
        <h1 className="text-2xl  font-bold text-center text-blue-700 mb-6">Search Student</h1>
        ...
        {submitStatus === 'student fetched successfully' && <p className="text-green-600 mt-4">Student fetched successfully!</p>}
        {submitStatus === 'student not found' && <p className="text-red-600 mt-4">Student not found.</p>}
      </form>
          ...
          <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow-md overflow-hidden">
            <thead>
              <tr className="bg-blue-500 text-white">
                ...
                <th className="py-3 px-4 border-b border-r border-gray-300 text-left">Actions</th>
              </tr>
            </thead>
            <tbody>
